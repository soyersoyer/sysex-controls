...
struct _KorgBookClass
{
  ...
  int (*read_scene) (snd_seq_t *seq, snd_seq_addr_t addr, uint8_t dev_id[4], uint8_t scene_id, uint8_t data[256], uint8_t *size);
  int (*write_scene) (snd_seq_t *seq, snd_seq_addr_t addr, uint8_t dev_id[4], uint8_t scene_id, uint8_t data[256], uint8_t size);
